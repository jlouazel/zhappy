...
void BaseApplication::createFrameListener(void)
{
  ...
  mDetailsPanel = mTrayMgr->createParamsPanel(OgreBites::TL_BOTTOMRIGHT, "DetailsPanel", 200, items);
  ...
  mTimePanel = mTrayMgr->createParamsPanel(OgreBites::TL_TOPRIGHT, "TimePanel", 200, items2);
  mTimePanel->setParamValue(0, toString(client->getTimeUnit()));
...
bool BaseApplication::frameRenderingQueued(const Ogre::FrameEvent& evt)
{
  ...
  for (std::map<int, Players * >::iterator it = client->getPlayers().begin(); it != client->getPlayers().end(); ++it)
	...
	p->setOldY(p->getOldY() - client->getWorld()->getHeight());
      ...
      if (abs(p->getX() - p->getOldX()) > 1)
	{
	  if (p->getX() > client->getWorld()->getWidth())
	    p->setOldX(p->getOldX() + client->getWorld()->getWidth());
	  else
	    p->setOldX(p->getOldX() - client->getWorld()->getWidth());
	}
      if (abs(p->getY() - p->getOldY()) > 1)
	{
	  if (p->getY() > client->getWorld()->getHeight())
	    p->setOldY(p->getOldY() + client->getWorld()->getHeight());
	  else
	    p->setOldY(p->getOldY() - client->getWorld()->getHeight());
	}
	...
	p->setOldY(p->getY());
      ...
      timeInterval = timeInterval * M_PI / 2;
      playerNode->yaw(Ogre::Radian(p->getOldDirection()));

      if (abs(p->getRealDirection() - p->getOldDirection()) > M_PI)
	{
	  if (p->getRealDirection() > 0)
	    p->setOldDirection(p->getOldDirection() + 2 * M_PI);
	  else
	    p->setOldDirection(p->getOldDirection() - 2 * M_PI);
	}
      if (p->getRealDirection() <= p->getOldDirection() - timeInterval)
        p->setOldDirection(p->getOldDirection() - timeInterval);
      else if (p->getRealDirection() >= p->getOldDirection() + timeInterval)
        p->setOldDirection(p->getOldDirection() + timeInterval);
      else
        p->setOldDirection(p->getRealDirection());
...
bool BaseApplication::keyPressed( const OIS::KeyEvent &arg )
{
  ...
  if (arg.key == OIS::KC_F)   // toggle visibility of advanced frame stats
    ...
    {
      if (client->getTimeUnit() < 200)
	client->writeOnServer("sst " + toString(client->getTimeUnit() + 10) + "\n");
    }
  else if (arg.key == OIS::KC_SUBTRACT)
    {
      if (client->getTimeUnit() > 10)
	client->writeOnServer("sst " + toString(client->getTimeUnit() - 10) + "\n");
    }
